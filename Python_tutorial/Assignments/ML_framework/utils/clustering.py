...
def regress_plot(df, val): 
    plt.figure(figsize=(8,6))
    sns.regplot(x = df["CustomerID"], y = df[val])
    plt.show()

def box_plot(df, val):
    sns.boxplot(df[val])

def print_score(df, range_n_clusters):
...
def count_plot(df, val):
    pal = ["#682F2F","#B9C0C9", "#9F8A78","#F3AB60"]
    pl = sns.countplot(x=df[val], palette= pal)
